...
| OQ-052 | 高 | P4.3 前確認 | `ADMIN` 是全系統管理員，還是仍受 authorized company scope 限制？ | P4.2 沿用既有 server authorization、角色常數與 session contract，未改 RBAC 或 admin API。 | 決定 `admin/users/*` 是否可管理其他公司使用者，以及多公司使用者的管理邊界。 |
| OQ-053 | 中 | P4.4 前確認 | 新 UI 的 company context 是否一律以 session active company 為準？ | P4.2 沿用既有 company switch API 與 active company session contract，未移除業務頁面的既有 selector。 | 決定 URL `companyId` 的例外用途，以及 P4.4 移除頁面本地 selector 的遷移 convention。 |
| OQ-054 | 中 | P4.3／P4.4 前確認 | `PageHeader`／`PageContainer` 如何套用至既有業務頁面？ | P4.2 只建立代表性共用元件與 App Shell，未全面重構業務頁面。 | P4.3 先固定元件 contract，P4.4 前建立既有頁面的遷移順序與例外規則。 |
