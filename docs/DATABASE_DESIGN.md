...
文件狀態：P1～P3.3 正式資料模型已完成；P4.1、P4.2 已完成且未修改 schema 或 migration；下一正式階段為 P4.3；P5 尚未開始
...
- P4.2 已於 2026-07-31 完成，closure commit 為 `29e68fff4cbd005443c0d228563a81e36ecf403d`；authenticated App Shell、navigation、company switcher、user menu、breadcrumb、responsive shell 與 accessibility baseline 均未變更 Prisma schema、migration、RBAC mapping、session model、transaction、audit、idempotency 或 formal print。下一正式階段為 P4.3；P5 尚未開始。
...
P3.2a 已建立 `DeliveryNoteStatus`、`DeliveryNoteVoidSource`、`DeliveryNote`、`DeliveryNoteLine` 與 create-only `0010_p3_delivery_notes`，並在兩個獨立 fresh DB 完成 0001～0010、catalog 與 schema diff=0 驗證。0010 已受控部署本機 `erp` 並通過 migration、catalog 與 health Gate。P3.2b～P3.2e 的建立／查詢、replacement rebuild、ADMIN direct void、API、UI 與整合驗收均已完成。
...
P3.2c 沿用 0010 的非 `VOIDED` partial unique 與 replacement constraints，不建立 0011。Rebuild 固定先 lock order、再 lock current delivery note；取號後於同一 transaction 先將舊單改為 `VOIDED/ORDER_REVISION_REBUILD`，再建立指向舊單的新 `ACTIVE` 單。ADMIN direct void 同樣依 order→current note 順序鎖定，以 `ADMIN_DIRECT` 作廢並將 order 恢復 `CONFIRMED`。Header／line／order／audit 注入失敗均驗證可完整 rollback。
...
- 正式版本表及事件表不得提供 UPDATE／DELETE route；若需移除錯誤資料，必須另立受控修正決議，不得以 hard delete 取代歷程。
- 現有銷貨單沒有獨立稅額欄位，且總額等於小計加運費。P3.3 不新增或推算 `tax_amount`；第一版 PDF 以固定文字「稅額：未分列」揭露。未來數值稅額需獨立交易規則及 migration。
...
- V0.13（2026-07-31，P4.2 完成同步）：記錄 P4.2 closure commit 與 UI 架構完成範圍；確認 schema／migration 0 變更，下一正式階段為 P4.3，P5 尚未開始。
